//
// Created by Edmond on 02/05/2020.
//

#ifndef UNTITLED5_KEPTPIECE_MOVEMENT_H
#define UNTITLED5_KEPTPIECE_MOVEMENT_H

#include "game_init.h"
//Functions controlling when a kept piece is added to the game
void kept(square board[BOARD_SIZE][BOARD_SIZE],player *myPlayer);
void keptPiece_stacking(square board[BOARD_SIZE][BOARD_SIZE],int row,int column,player *myPlayer);
#endif //UNTITLED5_KEPTPIECE_MOVEMENT_H
